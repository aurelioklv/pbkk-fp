...
class WisataSeeder extends Seeder
{
    public function run()
    {
        ...
        $faker->seed(42);
        $category = ['Alam', 'Religi', 'Sejarah'];
        for ($i = 0; $i < 7; $i++) {
            $randomPictureId = $faker->numberBetween(1, 999);
            $data = [
                'name' => $faker->city,
                'description' => $faker->realText(200, 2),
                'price' => $faker->randomNumber(5, true),
                'category' => $category[$faker->numberBetween(0, 2)],
                'image_url' => 'https://picsum.photos/seed/wisata' . $randomPictureId . '/370/370',
